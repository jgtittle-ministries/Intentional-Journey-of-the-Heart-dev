...
| **Mustard Seed Resolution** (V1.Exp8 open question) — *analysis in progress* | Workable *now* by one person as a scriptural-analysis project — catalog the miracle and answered-prayer narratives across the OT, Gospels, Acts, and Epistles by what is said about the faith involved. Lower empirical payoff, but it unblocks the functional form of V3.Exp3's `f()` and `h()` factors, and it is the only quantitative-side trail runnable solo. Phase 1 (coded corpus, ~59 entries) and the Phase 2b adversarial pass are recorded under `_implementation-notes/mustard-seed-resolution/`; the first pass rejects pure proportionality and favours the synthesis (a quality threshold with above-floor magnitude carried by resonance/authority/obedient-capacity, bounded by sovereignty). Pending: independent adversarial confirmation, then a Proposal-Template resolution. |
