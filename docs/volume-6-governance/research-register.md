...
| **Mustard Seed Resolution** (V1.Exp8 open question) — *resolution in 14-day comment period* | Workable *now* by one person as a scriptural-analysis project — catalog the miracle and answered-prayer narratives across the OT, Gospels, Acts, and Epistles by what is said about the faith involved. Lower empirical payoff, but it unblocks the functional form of V3.Exp3's `f()` and `h()` factors, and it is the only quantitative-side trail runnable solo. The Council-facing backstory and reasoning are written up in [The Mustard Seed Question — A Worked Resolution](mustard-seed-worked-resolution.md). Phases 1–6 plus an independent adversarial pass (coded corpus of ~59 entries, two "argue-for-B" passes — one of them by a reviewer blind to the analysis, discriminator tables and adjudication, cross-tradition check, and a draft Proposal-Template resolution) are recorded under `_implementation-notes/mustard-seed-resolution/`. The analysis rejects pure proportionality (Model B) and adopts the synthesis: a quality threshold on genuine trust (`f()` as a near-step floor) with above-floor magnitude carried by resonance, authority, and obedient capacity (`g()`/`h()`), bounded by divine sovereignty; the differential "gift of faith" (1 Cor 12:9) is preserved as a separate open question. The independent pass corroborated this (~80%) and added four adopted refinements (engaging the omitted faith-magnitude texts, an honest weighting of the κατά grammar, and rewording the gift-of-faith claim and the persistence/faith partition). The founder sponsors the V3.Exp3 research-program-track entry, and the proposer's experiential corroboration (§3b — prayer-as-rhythm, "pray without ceasing," now shared corporately as "two or three gathered") is in; the draft is **submission-ready** pending only the proposer's signature. **Enacted in the claim registry 2026-06-01:** the V1.Exp8 open question is now in the 14-day comment period (closes 2026-06-15), V1.Exp8 confidence raised 70→80 (founder, effective on close), V3.Exp3 entered the research-program track (founder-sponsored, six-month window), the gift-of-faith is preserved as a new open question (V3.Exp3.Open2), and V3.Exp8 is flagged for review at the next calibration cycle. |
