...
Now the detail view — showing how the schema's fan-in structure works for the single most dependency-rich claim in the corpus, V3.Exp3 (the spiritual force equation). This is what a Council member should look at when deciding whether to approve a change to any of its upstream claims. (Note: the Vol 3 nine-author review revised V3.Exp3 to lead with a non-operable sovereignty term — `F_s = G × [ f × g × h × i ]`, where `G` is God's free self-giving and the four bracketed factors are conditions of reception, not causes of production. `G` is not an upstream dependency in the schema sense; the ten dependencies remain the bracketed factors' Vol 1/2 sources, and the claim's confidence tier is unchanged.) Two things worth saying about these:
