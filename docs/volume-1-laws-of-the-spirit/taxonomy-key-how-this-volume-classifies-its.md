...
## Meta / Cycle Laws (Integrative)
A few laws are not single relationships at all: they *bind a family of other laws into one working whole* — sometimes as a **cycle** that sequences several laws over time (for example: image → hear → speak in truth and love → align → be weighed → grow into Christ), sometimes as a standing structure that governs or holds a cluster together. These are second-order laws — laws *about how the other laws behave as a set* — and I keep them in their own type rather than forcing them into a peer row. A cycle that moves through Spirit, Mind, Body, and Frame at once has no honest single-cell address; instead of a Period/Group/Direction triple it cannot truthfully carry, an integrative law simply lists the laws it gathers. Vol 3 already models most of this second-order structure — the [Causal-Loop Set](../volume-3-quantitative-framework/the-complete-causal-loop-set.md) draws the cycles, and the [Role Atlas](../volume-3-quantitative-framework/the-role-atlas-where-each-law-lives.md) draws the standing frame — and Vol 5's Meta-Law Layer is the index that names each one and points to where it already lives (Vol 5 sorts them into governing, binding, pathway, and frame kinds).
