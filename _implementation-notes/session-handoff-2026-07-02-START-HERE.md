...
**2026-07-04 follow-up (John's review feedback):** Periodic Table page restored to the slim whole-table-in-one-glance format via a new `compact` table mode — reader.js marks pages in `COMPACT_TABLE_PAGES` (currently the Periodic Table + the Formation Companion); their tables force-squeeze with `table-layout: fixed` + 12.5px type so the full grid always fits the column; phones fall back to the boxed scroll. To give any other reference-matrix page the same treatment, add its path to `COMPACT_TABLE_PAGES` in reader.js.

**2026-07-04 MIRRORED to prod on John's "please mirror": dev `3f05b51` → prod `41bb4dc`.** Batch = full readability pass (PR #11) + compact-table follow-ups + JD/other-session content commit `bcfe0bc` (Vol 3 Exp 2 duplicate-callout merge). Baselines all checked (one EOL-only false alarm on exploration-02); root html Repo links transformed in place; prod `search-index.js` regenerated (182 chapters); standing divergences preserved. Prod Pages verified live (FC Development Matrix + acronyms table render in the new formats).

**2026-07-04 CONTENT-SIDE CLEANUP (from the readability audit's flag list):** dev `fc3aced` MIRRORED to prod `98a25b9` (baseline-checked, staged==dev, prod index regenerated). Markup-typo fixes ×9 chapters (wording verified byte-identical after markup strip); • bullets → real lists across all 20 Vol-5 protocol chapters; volume-TOC "Title — Title" echoes stripped (102 entries, real descriptions kept); a32 Mini-Hot-Seat heading; `LotS-*` code span + reader.js code-span masking (inline code now opaque to emphasis parsing); Introduction.html Lewis pull-quote dedupe (kept the in-flow copy); chapter-outline 3-line clamp; congregation-phases + energy-landscape label nudges. **DEV-ONLY pending John's review (commit `41c0fed`, NOT mirrored): (1) FL.XLIV Mirror block split into 3 italic paragraphs (wording untouched); (2) Connecting-the-Dots "Some Open Trails" image — watermarked ArkansasOutside photo REPLACED with CC0 woodland-path photo (`open-trail-into-the-woods.jpg`, openverse/WP-photo-directory, 1600px q82); old image-015.jpeg deleted.** Remaining content flags (untouched, next session): low-res/clip-art image replacements (exploration-01-heart-soil sower, exploration-06 word-cloud, exploration-02a, -03 ×2, -04, -08 thumbnail, case-for-quantification, TFT PowerPoint slide exp-02, chemistry periodic-table photo, connecting-dots image-002 clip-art) — John's paid-source picks per the image-project workflow; overview-of-work PNG diagram clipped Op1/Mn1 chips (raster, needs regeneration).

**2026-07-04 review items MIRRORED on John's word: prod `0e8b243`** (FL.XLIV 3-paragraph Mirror + Open-Trails CC0 photo; old image-015.jpeg deleted both sides, live-verified 200/404). Dev and prod fully in sync again (only standing divergences remain).
